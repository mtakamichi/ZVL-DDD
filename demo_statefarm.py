#!/usr/bin/env python3
"""Zero-shot distracted driver detection on StateFarm.
...
living in :mod:`src.utils`. It mirrors ``demo_sam_dd.py`` -- only the dataset
differs.

Example:
    uv run python demo_statefarm.py \\
        --data-root /path/to/state-farm-distracted-driver-detection \\
...
def main():
    args = utils.build_demo_parser(DATASET, "Zero-shot distracted driver detection on StateFarm.").parse_args()
